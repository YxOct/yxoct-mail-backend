...
This repository also manages the non-secret deployment definitions for Stalwart, its consistent offline backup, and off-host backup retention.
...
- `deploy/remote-backup/`: retention cleanup on the backup server.
...
## Deploy remote retention cleanup
...
Create `/etc/default/yxoct-mail-backup-cleanup` from `deploy/remote-backup/yxoct-mail-backup-cleanup.env.example` only when it does not already exist. Never overwrite the server's real environment file during deployment.
...
```bash
systemctl list-timers 'stalwart-backup*' 'yxoct-mail-backup-cleanup*'
